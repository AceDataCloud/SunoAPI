...
Suno allows us to create secondary works from generated music and obtain the MIDI of the music. This document explains the integration methods for the relevant API.

The core input parameter for this API is `audio_id`, which is the official song ID generated, typically corresponding to the full track separated song ID (where the `action` parameter in [song generation](https://platform.acedata.cloud/documents/suno-audios) is `all_stems`). Optionally, it also supports a `callback_url` for asynchronous callback addresses.
