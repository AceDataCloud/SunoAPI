...
The core input parameter of this API is `audio_id`, which is the official song ID generated, usually corresponding to the full track separation song ID (where the parameter `action` is `all_stems` in [song generation](https://platform.acedata.cloud/documents/suno-audios)). Optionally, it also supports `callback_url` for asynchronous callback addresses.
