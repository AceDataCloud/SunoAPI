...
This document will provide detailed instructions for integrating the Suno Tasks API, helping you easily leverage the powerful features of this API. With the Suno Tasks API, you can easily query the execution status of tasks from the Suno Lyrics Generation API or Suno Audios Generation API.
...
We will take a task ID returned by the Suno Audios Generation API as an example to demonstrate how to use this API. Suppose we have a task ID: eae26f89-b64b-404d-a80c-761996660b1c, we will now demonstrate how to pass in a task ID.
...
You will notice that various language codes have been automatically generated on the right side of the page, as shown in the image below:
...
Upon a successful request, the API will return the details of the song task here. For example:
```json
{
