...
With the singer style ID, we can then use the [Suno Audios Generation API](https://platform.acedata.cloud/documents/4da95d9d-7722-4a72-857d-bf6be86036e9) for custom song generation. For example, pass `action` as `artist_consistency`, while `audio_id` is the uploaded song ID, and also upload the parameter `persona_id`, which is the singer style ID returned above, to generate a new song based on the reference singer style.